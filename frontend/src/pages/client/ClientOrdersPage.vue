              ...
              <template v-slot:body-cell-paidDate="props">
                <q-td :props="props">
                  <q-radio
                    ...
                    disable
                    :color="
                      Boolean(props.row.paidDate) ? 'positive' : 'negative'
                    "
                    keep-color
                  />
                </q-td>
...
import ClientAuthGuard from "src/components/ClientAuthGuard.vue";
...
// TODO: dup with WaiterOrdersPage
...
const channels = computed(() => {
  return userId.value === ANONYMOUS.userId
    ? []
    : [CHANNELS.ORDERS_OF_CLIENT(userId.value)];
});
